/* ============================================================
   AI Essentials for Business — shared site behaviour
   - mode toggle (Present / Self-paced), persisted
   - capture: autosave any [data-capture] field to localStorage,
     surfaced later in My AI Toolkit
   - subnav scroll-spy
   - reduced-motion aware helpers
   ============================================================ */
(function(){
  const STORE_MODE = 'aie.mode';
  const STORE_CAP  = 'aie.capture';
  ...
  window.AIE.getCapture = loadCap;
  window.AIE.setCaptureValue = function(key,val){
  ...
  function recordLast(){
    ...
    if(p && /^module-\d+$/.test(p)){
      try{ localStorage.setItem('aie.last', p); }catch(e){}
    }
  }

  /* ---------- cheat-sheet lightbox (works for images AND native HTML cards) ---------- */
  function initLightbox(){
    const sheets=document.querySelectorAll('[data-sheet]');
    if(!sheets.length) return;
    let lastFocus=null;
    let lb=document.getElementById('aie-lightbox');
    if(!lb){
      lb=document.createElement('div'); lb.id='aie-lightbox'; lb.className='lightbox';
      ...
      lb.innerHTML='<div class="lbinner"><button class="lbclose" aria-label="Close viewer">&times;</button>'
        +'<div class="lbcard" id="aie-lbcard"></div></div>';
    ...
    const cardHost=document.getElementById('aie-lbcard'), closeBtn=lb.querySelector('.lbclose');
    sheets.forEach(el=>{
      el.addEventListener('click',()=>{
        const tmpl=el.querySelector('template');
        cardHost.innerHTML = tmpl ? tmpl.innerHTML : el.innerHTML;
